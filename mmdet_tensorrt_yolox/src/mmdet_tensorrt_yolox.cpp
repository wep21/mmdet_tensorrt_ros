...
: score_threshold_(score_threshold)
{
  trt_common_ = std::make_unique<tensorrt_common::TrtCommon>(
    model_path, precision, nullptr, batch_config, max_workspace_size, plugin_paths);
  trt_common_->setup();
...
bool MmdetTrtYolox::doInference(const std::vector<cv::Mat> & images, ObjectArrays & objects)
{
  ...
  std::vector<void *> buffers = {input_d_.get(), out_dets_d_.get(), out_labels_d_.get()};
  ...
  CHECK_CUDA_ERROR(cudaMemcpyAsync(
    out_dets.get(), out_dets_d_.get(), sizeof(float) * 5 * batch_size * max_detections_,
    cudaMemcpyDeviceToHost, *stream_));
  CHECK_CUDA_ERROR(cudaMemcpyAsync(
    out_labels.get(), out_labels_d_.get(), sizeof(int32_t) * batch_size * max_detections_,
    cudaMemcpyDeviceToHost, *stream_));
  ...
  for (size_t i = 0; i < batch_size; ++i) {
    ...
    for (size_t j = 0; j < max_detections_; ++j) {
      ...
      if (object.score < score_threshold_) {
        break;
      }
